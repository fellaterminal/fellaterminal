...
class Agent:
    ...
    def make_decision(self, prompt: str, project_name: str) -> str:
        ...
        for item in decision:
            ...
            self.project_manager.add_message_from_fellaterminal(project_name, reply)
            ...
            if function == "git_clone":
                ...
                self.project_manager.add_message_from_fellaterminal(project_name, response)
    ...
    def subsequent_execute(self, prompt: str, project_name: str):
        ...
        new_message['from_fellaterminal'] = False
    ...
    def execute(self, prompt: str, project_name: str) -> str:
        ...
        self.project_manager.add_message_from_fellaterminal(project_name, reply)
        self.project_manager.add_message_from_fellaterminal(project_name, json.dumps(plans, indent=4))
        # self.project_manager.add_message_from_fellaterminal(project_name, f"In summary: {summary}")
        ...
        if (queries and len(queries) > 0) or ask_user != "":
            self.project_manager.add_message_from_fellaterminal(
                project_name,
        ...
        if ask_user != "" and ask_user is not None:
            self.project_manager.add_message_from_fellaterminal(project_name, ask_user)
            self.agent_state.set_agent_active(project_name, False)
            ...
            while not got_user_query:
                ...
                if latest_message_from_user and validate_last_message_is_from_user:
                    ...
                    got_user_query = True
                    self.project_manager.add_message_from_fellaterminal(project_name, "Thanks! 🙌")
                time.sleep(5)
