    ...
    {#each $messages as message}
      <div class="flex items-start gap-2 px-2 py-4">
        {#if message.from_fellaterminal}
          <img
            src="/assets/fellaterminal-avatar.png"
            alt="fellaterminal's Avatar"
        ...
        <div class="flex flex-col w-full text-sm">
          <p class="text-xs text-gray-400">
            {message.from_fellaterminal ? "fellaterminal" : "You"}
            <span class="timestamp">{new Date(message.timestamp).toLocaleTimeString()}</span>
          </p>
          {#if message.from_fellaterminal && message.message.startsWith("{")}
